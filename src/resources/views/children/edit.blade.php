        ...
        <div class="row">
            <div class="col-12">
                <div class="card mt-3">
                    <div class="card-body">
                        <div class="card-text">
                            <form method="POST" action="{{ route('children.update', ['child' => $child ]) }}" enctype="multipart/form-data">
                                @method('PATCH')
                                @csrf
                                <div class="form-group">
                                    <h8><p class="mt-3">お子さまのお名前</p></h8>
                                    @if($errors->has('name')) <span class="text-danger">{{ $errors->first('name') }}</span> @endif
                                    <textarea name="name" class="form-control" rows="1" placeholder="お子さまのお名前">{{ $child->name ?? old('name') }}</textarea>
